...
class SettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.combo_lang.addItem("[EN] English (US)", "en")
        self.combo_lang.addItem("[FR] Français (AZERTY)", "fr")
        self.combo_lang.addItem("[AR] العربية (Arabic)", "ar")
        current_lang = self.config_manager.get("language", "en")
        idx_l = self.combo_lang.findData(current_lang)
        if idx_l >= 0:
        ...
        self.combo_icon.addItem("❖ Diamond", "❖")
        ...
        # 3. Toggle Button Size
        lbl_tsize = QLabel("Floating Toggle Button Size:")
        lbl_tsize.setStyleSheet("font-size: 11px; color: #a6adc8;")
        self.combo_tsize = QComboBox()
        self.combo_tsize.setFocusPolicy(Qt.FocusPolicy.NoFocus)
        self.combo_tsize.addItem("Mini (30px)", 30)
        self.combo_tsize.addItem("Small (38px - Default)", 38)
        self.combo_tsize.addItem("Medium (48px)", 48)
        self.combo_tsize.addItem("Large (58px)", 58)
        current_tsize = self.config_manager.get("toggle_size", 38)
        idx_tsize = self.combo_tsize.findData(current_tsize)
        if idx_tsize >= 0:
            self.combo_tsize.setCurrentIndex(idx_tsize)
        layout.addWidget(lbl_tsize)
        layout.addWidget(self.combo_tsize)

        # 4. Keyboard Default Scale
        lbl_scale = QLabel("Keyboard Window Scale:")
    ...
    def save_settings(self):
        ...
        selected_tsize = self.combo_tsize.currentData()
